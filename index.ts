/**
 * OpenClaw Memory (LanceDB) Plugin — Custom Edition
 *
    ...
    const mapped = results.map((row) => {
      ...
      return {
        entry: {
          ...
          agentId: (row.agentId as string) || "shared",
        },
        score,
      };
    });
...
const memoryPlugin = {
  id: "memory-lancedb",
  name: "Memory (LanceDB)",
  description: "LanceDB-backed long-term memory with custom embedding support and multi-agent isolation",
    ...
    api.registerTool(
      {
        ...
        label: "Memory Recall",
        description:
          "Search long-term memories. Pass your agentId for multi-agent memory isolation. Use for user preferences, past decisions, or previously discussed topics.",
        parameters: Type.Object({
          query: Type.String({ description: "Search query" }),
          limit: Type.Optional(Type.Number({ description: "Max results (default: 5)" })),
          agentId: Type.Optional(Type.String({ description: "Agent ID for memory isolation. Pass your agent name to only recall your own memories." })),
        }),
        async execute(_toolCallId, params) {
          const { query, limit = 5, agentId } = params as { query: string; limit?: number; agentId?: string };
    ...
    api.registerTool(
      {
        ...
        label: "Memory Store",
        description:
          "Save important information to long-term memory. Pass your agentId for multi-agent memory isolation. Use for preferences, facts, decisions.",
        parameters: Type.Object({
          ...
          agentId: Type.Optional(Type.String({ description: "Agent ID for memory isolation. Pass your agent name to tag this memory as yours." })),
        }),
        async execute(_toolCallId, params) {
          const {
            text,
